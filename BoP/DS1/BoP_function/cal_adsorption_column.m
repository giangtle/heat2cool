function [out_avg, out_dynamic, T_bed_f, t_ads] = cal_adsorption_column(in, T_bed_0, mSi, yH2O_ads)
% Adsorption process.
[~, liq_stream] = cal_isenthalpic_condensation(in);
...
tspan = [0 (mSi*capacity/(in.yH2O*in.n*H2O.MW*0.01))];
y0 = [0, T_bed_0];
opt = odeset('Events', @myEvent);
[t, y] = ode15s(@(t, y) find_prime(t, y, in), tspan, y0, opt);
% adsorption time:
t_ads = t(end);
...
T_bed_m = y(:,2);
out_dynamic = cal_out_stream(in, T_bed_m, yH2O_ads);
out_dynamic.t = t;

% out average stream:
out_avg = cal_out_stream(in, T_bed_m, yH2O_ads);
out_avg.T = trapz(t, y(:,2))/t_ads;
